...
def invite_account_to_organization(connection, module):
    params = {
        "Target": {
            "Id": module.params.get("target_id"),
            "Type": module.params.get("target_type"),
        },
    ...
    try:
        response = connection.invite_account_to_organization(**kwargs)
        changed = True
    except Exception as e:
        module.fail_json_aws(e)

    invite_account_to_organization_status = _parse_response(response)

    module.exit_json(changed=changed, result=invite_account_to_organization_status)


def main():
    argument_spec = dict(
        target_id=dict(required=True),
        target_type=dict(required=True),
        notes=dict(required=False, default=None),
    ...
    invite_account_to_organization(connection, module)

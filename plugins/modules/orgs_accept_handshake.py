...
def accept_handshake(connection, module):
    params = {
        "HandshakeId": module.params.get("handshake_id"),
    }
    changed = False
    ...
    try:
        response = connection.accept_handshake(**kwargs)
        changed = True
    except Exception as e:
        module.fail_json_aws(e)

    accept_handshake_status = _parse_response(response)

    module.exit_json(changed=changed, result=accept_handshake_status)


def main():
    argument_spec = dict(
        handshake_id=dict(required=True),
    )
    ...
    accept_handshake(connection, module)

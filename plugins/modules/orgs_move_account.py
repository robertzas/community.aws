...
def move_account(connection, module):
    params = {
        "AccountId": module.params.get("account_id"),
        "SourceParentId": module.params.get("source_parent_id"),
        "DestinationParentId": module.params.get("destination_parent_id"),
    }
    changed = False
    ...
    try:
        response = connection.move_account(**kwargs)
        changed = True
    except Exception as e:
        module.fail_json_aws(e)

    move_account_results = _parse_response(response)

    module.exit_json(changed=changed, result=move_account_results)


def main():
    argument_spec = dict(
        account_id=dict(required=True),
        source_parent_id=dict(required=True),
        destination_parent_id=dict(required=True),
    )
    ...
    move_account(connection, module)
